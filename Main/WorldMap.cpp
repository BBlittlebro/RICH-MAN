...
void WorldMap::Create_Map(){
    ...
    while(fgets(temp, 100, fp) != NULL){        //讀檔案
        ...
        if(type_and_name[0][0] == 'U'){
            ...
            int temp_money_list[MaxLv];
            for(int i = 2; i < 7; ++i){
                temp_money_list[i-2]=temp_money[i];
            }
            units_[unit_index]->SetFineList(temp_money_list,MaxLv);
...
void WorldMap::Display() const{
    ...
    for(int i = 0; i < number_units/2; ++i){
        ...
        printf("=");
        units_[i]->Display();
        if(units_[i]->Type()=='C'&&units_[i]->Host()!=Init){
            int have=0;
            for(int t=0;t<number_units;++t)
                if(units_[t]->Host()==units_[i]->Host()&&units_[t]->Type()=='C')
                    have++;
            printf("%d            ",have);
        }
        //後半部
        ...
        if(units_[number_units-1 -i]->Type()=='C'&&units_[number_units-1 -i]->Host()!=Init){
            int have=0;
            for(int t=0;t<number_units;++t)
                if(units_[t]->Host()==units_[number_units-1 -i]->Host()&&units_[t]->Type()=='C')
                    have++;
            printf("%d            ",have);
        }
        printf("\n");
